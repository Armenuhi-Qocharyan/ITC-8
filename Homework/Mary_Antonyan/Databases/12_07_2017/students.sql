...
--
-- Temporary table structure for view `participation`
--

DROP TABLE IF EXISTS `participation`;
/*!50001 DROP VIEW IF EXISTS `participation`*/;
SET @saved_cs_client     = @@character_set_client;
SET character_set_client = utf8;
/*!50001 CREATE VIEW `participation` AS SELECT 
 1 AS `Name`,
 1 AS `AVG`,
 1 AS `Count`*/;
...
--
-- Final view structure for view `participation`
--
...
/*!50001 VIEW `participation` AS select `st`.`Name` AS `Name`,avg(`e`.`Score`) AS `AVG`,count(`e`.`Score`) AS `Count` from (`exams` `e` join `students` `st`) where (`e`.`Student_id` = `st`.`id`) group by `st`.`Name` order by `Count` desc */;
...
/*!50001 CREATE ALGORITHM=UNDEFINED */
/*!50013 DEFINER=`root`@`localhost` SQL SECURITY DEFINER */
/*!50001 VIEW `statistics` AS select `sb`.`Title` AS `Subject`,max(`e`.`Score`) AS `Maximum`,min(`e`.`Score`) AS `Minimum`,avg(`e`.`Score`) AS `Average` from (`exams` `e` join `subjects` `sb`) where (`e`.`Subject_id` = `sb`.`id`) group by `sb`.`Title` */;
/*!50001 SET character_set_client      = @saved_cs_client */;
/*!50001 SET character_set_results     = @saved_cs_results */;
/*!50001 SET collation_connection      = @saved_col_connection */;
...
-- Dump completed on 2017-07-12 23:28:27
