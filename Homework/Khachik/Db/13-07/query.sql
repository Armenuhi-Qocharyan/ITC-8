...
select name, surname, ITC, AVG(score) as average_score  from exams, students  inner join itc on itc_id=itc.id where student_id=students.id group by name, surname, itc;
...
select name, surname, ITC, MIN(score), MAX(score), AVG(score),COUNT(DISTINCT itc)  from exams, students  inner join itc on itc_id=itc.id where student_id=students.id group by name, surname, ITC;
